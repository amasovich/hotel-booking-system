...
Проект реализует микросервисную систему бронирования отелей на базе **Spring Boot**:
**API Gateway + Eureka (Service Discovery) + Hotel Service + Booking Service**.

Ключевые возможности:
- **Бронирование как распределённая транзакция (Saga)**: `booking-service` подтверждает доступность номера в `hotel-service` через internal endpoints `confirm/release` с компенсацией при ошибках.
- **Идемпотентность по `X-Request-Id`**: повторный запрос не создаёт дубликаты и не вызывает внешние операции повторно.
- **Алгоритм рекомендаций номера** (`recommend`) и управление состоянием номера (lock/release), включая бизнес-счётчик `timesBooked`.
- **Безопасность (JWT + RBAC)**: разграничение прав USER/ADMIN и защита внутренних endpoint’ов.
- **Тестовое подтверждение**: интеграционные тесты JUnit + E2E Postman-коллекция через gateway.
...
%% ===== Nodes =====
Client["Client"]:::svc
E["discovery-server<br/>Eureka<br/>Port:8761"]:::infra
G["api-gateway<br/>Port:8080<br/>Public API<br/>JWT + RBAC<br/>X-Request-Id"]:::gateway
B["booking-service<br/>Port:8081<br/>Users + bookings<br/>Saga<br/>Retries + timeouts<br/>Idempotency X-Request-Id"]:::svc

subgraph HS["hotel-service Port:8082"]
direction TB
...
## Автор
Выполнено Березняком Владимиром
в рамках учебного задания МИФИ.
